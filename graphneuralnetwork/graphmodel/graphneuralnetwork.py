...
class graphneuralnetwork:
    ...
    def forward(self, nnodes, adj,feat, W = None, A = None, b = None):
        """
        ...
        for i in range(adj.shape[0]):
            ...
            #print(np.tile(feat,[nnodes[i],1]))
            #print(feat)
            X.append(np.tile(feat[i],1))
            ...
            #output = p
            slist.append(s)
            outputlist.append(float(output))
    ...
    def loss(self,s,y):
        ...
        for i in range (len(s)):
            ...
            loss=np.square(np.subtract(s[i], y[i])).mean()
            #print(loss)
    ...
    def backward(self,loss,y,epsilon):
        ...
        for i in range (self.D):
            for j in range (self.D):
                ...
                sep,_ = self.forward(self.tempnnodes,self.tempadj,self.tempfeatures,W=Wepsilon)
        ...
        for i in range (self.D):
            ...
            sep,_ = self.forward(self.tempnnodes,self.tempadj,self.tempfeatures,A=Aepsilon)
            lossep = self.loss(sep,y)   
        ...
        sep,_ = self.forward(self.tempnnodes,self.tempadj,self.tempfeatures,b=bepsilon)
        lossep = self.loss(sep,y) 
...
features_list={'1000234_features.txt':['O','Pd','Ba','Cu','Y'],
               '1000031_features.txt':['Y','O','Cu','Ba'],
               '6_features.txt':['O','Sr','Ru','Gd','Cu','Ce'],
               '5_features.txt':['O','Li','Fe','H','Se'],
               '4_features.txt':['O','Cu','Ce','Nd'],
               '3_features.txt':['O','S','La','F','Bi'],
               '2_features.txt':['O','Ba','Cu','Y'],
               '1_features.txt':['H','As','Fe','Ba']
    }
